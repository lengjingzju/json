...
/*
 * @brief   the number value type
 * @note    The definition is consistent with the JSON object type in json.h
 */
/*
 * @brief   数值类型
 * @note    定义和json.h中的对象类型一致
 */
typedef enum {
    JNUM_NULL,
    JNUM_BOOL,
 ...
 * @note    The definition is consistent with the JSON number object value in json.h
 */
/*
 * @brief   数值
 * @note    定义和json.h中的数值对象值一致
 */
typedef union {
    bool vbool;
    int32_t vint;
...
/*
 * @brief   jnum_itoa  - Converts signed integer to decimal string
 ...
 * @param   num [IN] input number to convert
 * @param   buffer [OUT] buffer to store resulting string
 * @return  Length of converted string
 * @note    Buffer must have at least 27 characters capacity
 */
/*
 * @brief   jnum_itoa  - 打印有符号整数成十进制字符串
...
/*
 * @brief   Parses string to numerical value
 * @param   num [IN] input string to parse
 * @param   type [OUT] stores detected numerical type
 * @param   value [OUT] stores parsed numerical value
...
/*
 * @brief   解析字符串到数值
 * @param   num [IN] 被转换的字符串
 * @param   type [OUT] 存储数值的类型
 * @param   value [OUT] 存储数值的值
 * @return  返回解析的字符串长度
 * @note    jnum_parse_num不会跳过首部空字符，需要跳过空字符时请使用jnum_parse；
